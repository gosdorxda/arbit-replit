...
class MEXCAdapter(BaseAdapter):
    ...
    def fetch_usdt_tickers(self) -> List[NormalizedTicker]:
        try:
            ...
            logger.info(f"MEXC: Fetched {len(tickers)} USDT pairs")
            return tickers
            
        except requests.RequestException as e:
            logger.error(f"MEXC API error: {str(e)}")
            raise Exception(f"Failed to fetch MEXC data: {str(e)}")
        except Exception as e:
            logger.error(f"MEXC processing error: {str(e)}")
            raise
    ...
    def fetch_orderbook(self, symbol: str, limit: int = 20) -> NormalizedOrderbook:
        try:
            ...
            logger.error(f"MEXC orderbook API error: {str(e)}")
            raise Exception(f"Failed to fetch MEXC orderbook: {str(e)}")
        except Exception as e:
            logger.error(f"MEXC orderbook processing error: {str(e)}")
            raise
